...
df_process = 1/sqrt(Nfft) .* (fft(d, Nfft));
if mod(Nfft, 2) == 0
...
data_rotf = 1/sqrt(N) .* (fft(data_rot));

if mod(N, 2) == 0
    axis_rot = linspace(-N/2, N/2-1, N)/(N);
    vel_axis_rot = 2 * v_amb * axis_rot;
    f_axis_rot = linspace(0, 2*v_amb*2/lambda, N);
    
else
    
    vel_axis_rot = linspace(-v_amb, v_amb, N);
    f_axis_rot = linspace(0, 2*v_amb*2/lambda, N);
end
...
hold on;
color = 'r';
yl =  ['Processed Spectrum [dB] per rotation'];
plott(f_axis_rot, db(abs(data_rotf)), xl, yl, txt, 2, dtext, color, markers(2))
