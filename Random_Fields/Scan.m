...
F = 10^3 .* (rand(length(x_), length(y_)) + 1j .* rand(length(x_), length(y_)) - 0.5);
db(abs(min(min(F))))


figure; surface(x_.*1e-3, y_.*1e-3, (abs(F).')./2); colorbar; colormap('jet'); shading interp; colormap('jet');
...
F             = (ifft2(F));
% 
F             = imgaussfilt(real(F), 1);
N             = sqrt(max(abs(F)).^2./SNR) .* (rand(length(x), length(y)) + 1j .* rand(length(x), length(y)));
F             = F + N;

db(abs(min(min(F))))
figure; surface(x_.*1e-3, y_.*1e-3, db(abs(F).')./2); colorbar; colormap('jet');
