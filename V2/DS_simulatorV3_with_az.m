function [data, data_f, data_f_full, data_full] = DS_simulatorV3_with_az(SNR, m0, mu_max, sigma, n, v_amb, N, beta_scan_hd, theta)

%% N is the number of Doppler bins permitted for a specific rotation speed and radar beam width 
% mu = mu_max .* cos(beta_scan);
mu_hd = mu_max .* cos(beta_scan_hd) .* cos(theta);
...
%     S_(idx) = 1;
% else
S_ = m0/sqrt(2*pi*sigma^2) * exp(-(vel_axis - mu_hd).^2/(2*sigma^2));
% end 
