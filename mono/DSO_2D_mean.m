...
phi_hd = phi_0 + Omega(m)*t;

[r_gt, phi_gt] = meshgrid(r, phi_hd);

x_gt = r_gt .* 1e-3 .* cos(phi_gt) .* cos(theta);
y_gt = r_gt .* 1e-3 .* sin(phi_gt) .* cos(theta);
...
%% Ground Truth with Range Plot

if (Omega_rpm(m) == 1) && (Nr > 1)
    txt = ['Ground Truth Velocity at SNR = ', num2str(db(mean(SNR))/2), ' dB '];

    xl = 'x [km]';
    yl = 'y [km]';
    zl = 'V_{r} Mean [m.s^{-1}]';
    
    surplot_pcolor(x_gt, y_gt, mu_gt.', xl, yl, zl, txt); 
    caxis([min(min(mu_gt)) max(max(mu_gt))]);
end
...
if (MC > 1) && (c == 1) && (Nr == 1)
    ...
    marker = markers(m);
    f = figure(201); hold on;   f.Position = [10 10 1000 1000];
    color = 'k';
    yl =  ['\mu retrieved [m.s^{-1}], for 1 rotation'];
    plott(Vmean, squeeze(mu_re(c, 1, 1, m, :)), xl, yl, txt, 2, dtext, color, marker);
    ...
    marker = markers(m);
    f = figure(202); hold on;   f.Position = [10 10 1000 1000];
    color = 'k';
    yl =  ['\mu retrieved [m.s^{-1}], for 1 rotation'];
    plott(Vmean, squeeze(sigma_re(c, 1, 1, m, :)), xl, yl, txt, 2, dtext, color, marker);
end


Mu_re_avg = mean(mu_re, 1);
Sigma_re_avg = mean(sigma_re, 1);
Mu_re_std = std(mu_re, 1, 1);
Sigma_re_std = std(sigma_re, 1, 1);


%% Averaging in range axis 
if Nr > 1
    [~, numr] = find(r < 100);
    Num_N = length(numr);
    r_new = r(1:Num_N:end);
    ...
    for rng = 1:length(r_new)-1
        Mu_re_avg_rng(1, rng, :, m, n) = mean(Mu_re_avg(1, (rng - 1)*Num_N+1:rng*Num_N, :, m, n),  2);
        Sigma_re_avg_rng(1, rng, :, m, n) = mean(Sigma_re_avg(1, (rng - 1)*Num_N+1:rng*Num_N, :, m, n),  2);
        Mu_re_std_rng(1, rng, :, m, n) = std(Mu_re_avg(1, (rng - 1)*Num_N+1:rng*Num_N, :, m, n), 1, 2);
        Sigma_re_std_rng(1, rng, :, m, n) = std(Sigma_re_avg(1, (rng - 1)*Num_N+1:rng*Num_N, :, m, n), 1, 2);
    end
end
...
if (Nr == 1) && (length(Vmean) == 1) && (length(Vspread) == 1)
    txt = ['SNR = ', num2str(db(mean(SNR))/2), ' dB'];
    dtext = [' \Omega = ', num2str(Omega_rpm(m)), ' [rpm]'];
    yl = '\mu [m.s^{-1}]';
...
%     color = 'k';
    ...
    plott(phi_axis.*180/pi, squeeze(Mu_re_avg(1, 1, :, m, n)), xl, yl, txt, 2, dtext, color, marker)
elseif (length(Vmean) == 1) && (length(Vspread) == 1) 
    
    [r_, phi_axis_] = meshgrid(r_new(1:end -1), phi_axis);

    x = r_ .* 1e-3 .* cos(phi_axis_) .* cos(theta); y = r_ .* 1e-3 .* sin(phi_axis_) .* cos(theta);

    [r_orig, phi_axis_orig] = meshgrid(r, phi_axis);

    x_orig = r_orig .* 1e-3 .* cos(theta) .* cos(phi_axis_orig);
    y_orig = r_orig .* 1e-3 .* cos(theta) .* sin(phi_axis_orig);
    ...
    surplot_pcolor(x_orig, y_orig, squeeze(Mu_re_avg(1, :, :, m, n)).', xl, yl, zl, txt); 
    surplot_pcolor(x, y, squeeze(Mu_re_avg_rng(1, :, :, m, n)).', xl, yl, zl, txt); 
    surplot_pcolor(x, y, squeeze(Mu_re_std_rng(1, :, :, m, n)).', xl, yl, zl, txt); 
end
...
if (Nr == 1) && (length(Vmean) == 1) && (length(Vspread) == 1) 
    ...
    plott(phi_axis.*180/pi, squeeze(Sigma_re_avg(1, 1, :, m, n)), xl, yl, txt, 2, dtext, color, marker)
elseif (length(Vmean) == 1) && (length(Vspread) == 1) 
    txt = ['SNR = ', num2str(db(mean(SNR))/2), ' dB , \Omega = ', num2str(Omega_rpm(m)), ' [rpm]'];
    xl = 'x [km]';
    yl = 'y [km]';
    ...
    surplot_pcolor(x_orig, y_orig, squeeze(Sigma_re_avg(1, :, :, m, n)).', xl, yl, zl, txt);
    surplot_pcolor(x, y, squeeze(Sigma_re_avg_rng(1, :, :, m, n)).', xl, yl, zl, txt);
    surplot_pcolor(x, y, squeeze(Sigma_re_std_rng(1, :, :, m, n)).', xl, yl, zl, txt);
end
