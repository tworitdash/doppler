...
BW_deg = linspace(1, 180, 180);
Omega_rpm = 1;

for l = 1:length(BW_deg)
...
%% Signal generator and Doppler processing
    ...
    for i = 1:length(Omega_rpm)
       Omega = Omega_rpm(i) .* 2.*pi./60; % Angular velocity of the radar beam in rad/sec
        for k = 1:length(mean_Phi)
                ...
                hits_scan(l) = length(time_axis); % length of time axis

                delta_v(l) = lambda/(2*hits_scan(l)*PRT);% velocity resolution
                v_amb = lambda/(4*PRT);% Doppler ambiguity limits in velocity
                vel_axis(l).axis = linspace(-v_amb,v_amb,hits_scan(l));% velocity axis

                Nifft = hits_scan(l); % Number of DTFT points for ifft
    ...
    %             
                X(l).x = rand(1, hits_scan(l));  % Random number generator for the simulator amplitude for all the velocities
                Theta(l).theta = rand(1, hits_scan(l)) .* 2 .* pi; % Random phase generator for all the velocities (uncorelated with X in previous line)
            

                [Signal(l).sig(i, k, :), Signal(l).sig_doppler(i, k, :)] = Doppler_spectrum_TD(vel_axis(l).axis, mu, sigma, Nifft, SNR, lambda, X(l).x, Theta(l).theta); 
                % Time domain signal generator without considering any rotation speed
                % of the radar beam


                Signal(l).sig_with_Omega(i, k, :) = abs(squeeze(Signal(l).sig(i, k, :))) .* exp(1j * angle(squeeze(Signal(l).sig(i, k, :))).* cos(beta));% + noise(i).n;
                
                
                
                PT_integrand_simulator = abs(squeeze(Signal(l).sig_doppler(i, k, :))).^2 .* delta_v(i);
                PT_simulator = sum(PT_integrand_simulator); % Total power of the Doppler Spectrum

                v_mean_integrand_simulator = vel_axis(l).axis.' .* abs(squeeze(Signal(l).sig_doppler(i, k, :))).^2 .* delta_v(i);
                v_mean_simulator(l, i, k) = sum(v_mean_integrand_simulator) ./ PT_simulator; % Mean Doppler velocity 

                v_spread_integrand_simulator = (vel_axis(l).axis.' - v_mean_simulator(l, i, k)).^2 .* abs(squeeze(Signal(l).sig_doppler(i, k, :))).^2 .* delta_v(i);
                v_spread_simulator(l, i, k) = sqrt(sum(v_spread_integrand_simulator)./ PT_simulator); % Doppler spectrum width
...
for l = 1:length(BW)
    for i = 1:length(Omega_rpm)
        for k = 1:length(mean_Phi)
                ...
                Signal(l).doppler(i, k, :) = 1./sqrt(Nfft) .* (fftshift(fft(squeeze(Signal(l).sig_with_Omega(i, k, :))))); % Find the Doppler of the signal of the rotating radar
    ...
    %             vel_max(i, k) = vel_axis(i).axis(index);
                ...
                PT_integrand = abs(squeeze(Signal(l).doppler(i, k, :))).^2 .* delta_v(i);
                PT = sum(PT_integrand); % Total power of the Doppler Spectrum

                v_mean_integrand = vel_axis(l).axis.' .* abs(squeeze(Signal(l).doppler(i, k, :))).^2 .* delta_v(i);
                v_mean(l, i, k) = sum(v_mean_integrand) ./ PT; % Mean Doppler velocity 

                v_spread_integrand = (vel_axis(l).axis.' - squeeze(v_mean(l, i, k))).^2 .* abs(squeeze(Signal(l).doppler(i, k, :))).^2  .* delta_v(i);
...
for l = 1:length(BW)
    for i = 1:length(Omega_rpm)
        for k = 1:length(mean_Phi)
            v_meanE(l, i, k) = abs(squeeze(v_mean(l, i, k)) - v_mean_simulator(l, i, k))./squeeze(v_mean_simulator(l, i, k)) * 100;
            v_spreadE(l, i, k) = abs(squeeze(v_spread(l, i, k)) - v_spread_simulator(l, i, k))./squeeze(v_spread_simulator(l, i, k)) * 100;
        end
    end
end
...
figure; plot(BW_deg, mean_Phi, squeeze(v_meanE(:, 1, :)), 'color', [0.6350, 0.0780, 0.1840], 'LineWidth', 2)
...
figure; plot(BW_deg, mwan_Phi, squeeze(v_spreadE(:, 1, :)), 'color', [0.6350, 0.0780, 0.1840], 'LineWidth', 2)
