...
Omega_rpm = linspace(1, 120, 8); % RPM axis for the rotation of the radar
...
%% 
     time_axis_full = linspace(0, PRT.*N(i), N(i)); % High resolution time axis
     axis_full = linspace(-(N(i))/2, (N(i))/2, N(i))/(N(i)); 
     ...
     Signal(i).sig = abs(data(i).data) .* exp(1j .* unwrap(angle(data(i).data)) .* sin(eps + beta_wind - phi_axis) ./ (eps - Omega .* time_axis_full));
...
%                 .* exp(1j .* unwrap(angle(squeeze(data(i).data((k - 1)*hits_scan + 1: k*hits_scan)))) .* cos(beta_scan)));
         ...
         Signal(i).doppler(k, :) = 1./sqrt(hits_scan_1(i)) .* fftshift(fft(Signal(i).sig((k - 1)*hits_scan + 1: k*hits_scan), hits_scan_1(i))); % To be used for moments calculation
...
%% Compensation 
for i = 1:length(Omega_rpm)
    Omega(i) = Omega_rpm(i) .* 2 * pi ./ 60; % Angular speed in [radians/sec]
    
     T = BW/Omega(i); % Dwell time (Time for the radar beam to stay at one azimuth angle cell (beamwidth))
...
%% 
    ...
    Signal(i).sig_re = abs(Signal(i).sig) .* exp(1j .* unwrap(angle(Signal(i).sig)) .* (eps + Omega(i) .* time_axis_full)./(eps + sin(Omega(i) .* time_axis_full)));
    
    Signal(i).doppler_re = 1./sqrt(N(i)) .* fftshift(fft(Signal(i).sig_re)); % To be used for 
   axis_full = linspace(-(N(i))/2, (N(i))/2, N(i))/(N(i));
    vel_axis_full = 2.*v_amb.*axis_full; % High definition velocity axis 
    
    figure(101); plot(vel_axis_full, db(abs(Signal(i).doppler_re))); hold on;
end