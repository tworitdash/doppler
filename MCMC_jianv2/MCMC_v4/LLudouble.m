function ret = LLudouble(x, data, t_avail, x0, sigma_n, px)
...
%     a  = x(3);
    ...
    r2(1) = x0;
    Z_model(1) = (a1 .* exp(1j .* 4 .* pi/lambda .* r1(1)) + a2 .* exp(1j .* 4 .* pi/lambda .* r2(1)) ) ; % First sample echo model 
    ...
    for l = 2:length(t_avail)
           ...
           Z_model(l) = ( a1 .* exp(1j .* 4 .* pi/lambda .* r1(l))  + a2 .* exp(1j .* 4 .* pi/lambda .* r2(l)) );
    end
    Nt = length(data);
