...
Opt.fitnessfcn = @(x) Max_LikelihoodGaussvar1(x, Nscatters_p, Z_avail_vec, t_avail, x0, sigma_n, 1);
...
%     figure;
    ...
    set(bar_handle, {'DisplayName'}, {'GA algorithm'; 'Ground Truth'});
    legend;
    title(['Histogram of ', var(er).name], 'FontSize', 16);
    xlabel(var(er).name, 'FontSize', 16);
    grid on;
    
    print(['GA_Histogram_var1_', num2str(er)], '-depsc');
...
print('GA_Doppler_Spectrum_var1', '-depsc');