...
x0 = 10;  % Start position of the target
a = 1;
...
Opt.fitnessfcn = @(x) Max_LikelihoodN(x(1:Nscatters_p), Nscatters_p, Z_avail_vec, t_avail, x0, sigma_n, 1);
% Opt.objective = @(x) Max_LikelihoodN(x(1:Nscatters_p), Nscatters_p, Z_avail_vec, t_avail, x0, sigma_n, 1);
...
Opt.options = optimoptions(@ga, 'PlotFcn', {'gaplotbestf', 'gaplotbestindiv'}, 'Display', 'iter',... 
    ...
    true, 'FitnessLimit', Opt.fitnesslimit, 'FunctionTolerance', 0, 'MaxGenerations', 10000);
