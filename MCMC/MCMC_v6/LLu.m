% function ret = LLu(u, data, t_avail, r0, sigma_n, pu, Z, t_w, t_a)
...
%% Real and imaginary part likelihood 
    ...
    ret_re = (pu) -Nt/2 .* log(2*pi) - 1/2 * sqrt(2) .* (real(data) - real(Z_model).').' * inv(K) * (real(data) - real(Z_model).');
    ret_im = (pu) -Nt/2 .* log(2*pi) - 1/2 * sqrt(2) .* (imag(data) - imag(Z_model).').' * inv(K) * (imag(data) - imag(Z_model).');
