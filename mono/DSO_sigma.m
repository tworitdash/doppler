...
%     color = 'k';
    ...
    plott(phi_axis.*180/pi, squeeze(Mu_re_avg(1, 1, :, n)), xl, yl, txt, 2, dtext, color, marker)
% else
...
if (Nr == 1) && (length(Vspread) == 1)
    ...
    plott(phi_axis.*180/pi, squeeze(Sigma_re_avg(1, 1, :, n)), xl, yl, txt, 2, dtext, color, marker)
% else
%     txt = ['SNR = ', num2str(db(mean(SNR))/2), ' dB '];
%     xl = 'x [km]';
%     yl = 'y [km]';
...
plott(Vspread, squeeze(Mu_re_avg(1, 1, 1, m, :)), xl, yl, txt, 2, dtext, color, marker)
...
plott(Vspread, squeeze(Sigma_re_avg(1, 1, 1, m, :)), xl, yl, txt, 2, dtext, color, marker)
...
plott(Vspread, squeeze(Mu_re_std(1, 1, 1, m, :)), xl, yl, txt, 2, dtext, color, marker)
...
plott(Vspread, squeeze(Sigma_re_std(1, 1, 1, m, :)), xl, yl, txt, 2, dtext, color, marker)
