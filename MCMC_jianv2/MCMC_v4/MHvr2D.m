function [accepted, rejected, itern, E, sample_MC_seq, sample_all] = MHvr2D(E, iter, data, t_avail, N, x0, y0, sigma_n)
...
% updated
    ...
    for i = 1:iter
        ...
        [theta_new, pthetanew, ptheta] = TMu_uniformprior(E);     % x_new is a new sample drawn for u 
                                                % pxnew is the prior probability of x_new
                                                
                                                % px is the prior probability of x 

        sample_all(:, i) = theta_new;

        theta_lik = LLvr2D(theta, data, t_avail, N, x0, y0, sigma_n, pthetanew); % Likelihood of x wth data
        theta_new_lik = LLvr2D(theta_new, data, t_avail, N, x0, y0, sigma_n, ptheta); % Likelihood of x_new with data
...
%         if (acceptance(real(x_lik), real(x_new_lik))) || (acceptance(imag(x_lik), imag(x_new_lik)))
        if (acceptance(theta_lik, theta_new_lik))         
            ...
            itern = [itern i];
            sample_MC_seq(:, i) = theta_new;
            theta = theta_new;
            
%             
        else
            rejected = [rejected; theta_new];
            sample_MC_seq(:, i) = theta;
        end
    end
end