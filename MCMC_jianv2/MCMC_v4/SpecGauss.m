function [ZFFT, PT, mu, sigma, vel_axis, dv, v_amb] = SpecGauss(z_model, Nt, dt, lambda, SNR_db, isvelamb, isplot, mi, ci, mu_true, sigma_true, Nt_True, dv_obs)
...
if isvelamb == 1
    ZFFT = 1./sqrt(Nt) .* fftshift(fft(z_model, Nt));
else
    ZFFT = 1./sqrt(Nt) .* (fft(z_model, Nt));
end
...
%     hsnew = hs;
    ...
    if (mod(Nt, 2) == 0)
%     hsnew = hs+1;
    vel_axis = linspace(0, Nt, Nt)./Nt .* 2 .* v_amb;
%     vel_axis_hs = linspace(-v_amb, v_amb, hsnew);
    else
%     hsnew = hs;
    vel_axis = linspace(0, 2*v_amb, Nt);
    end
end
...
%% True length and Doppler resolution
...
dtext = ['Nt = ', num2str(Nt_True), ', dv = ', num2str(dv_obs), ' [m/s]', ', \mu_{re} = ', num2str(mu), ' [m/s]', ', \sigma_{re} = ', num2str(sigma), ' [m/s]', ...
