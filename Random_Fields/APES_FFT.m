function [H, beta, omega, g_mtr] = APES_FFT(sig, M, N_omg)
% Amplitude and Phase Estimation Method (APES) implemented by taking
% advantage of the FFT
%
% Parameter:
...
N = length(sig);
if N_omg<N-M+1
error('MyComponent:incorrectType','Error. \nA larger value must be used as the number of frequencies.')
end
R = zeros(M,M);
...
g_mtr = fft(sig_mtr, N_omg, 2) * diag(exp(-ii * omega * M)) / (N-M+1);
% g_mtr = fft(sig_mtr, N_omg, 2) ;
