...
for i = 1:Nr
    for k = 1:Nphi
        ...
        PT(i, k, m, n) = sum(PT_i .* dv);
        mu_re_i = vel_axis_hs.' .* squeeze(abs(S1_norm(i, k, :))).^2;
        mu_re(i, k, m, n) = sum(mu_re_i .* dv)./PT(i, k, m, n);
        sigma_re_i = (vel_axis_hs.' - mu_re(i, k, m, n)).^2 .* squeeze(abs(S1_norm(i, k, :))).^2 .* dv;
        sigma_re(i, k, m, n) = sqrt(sum(sigma_re_i)./PT(i, k, m, n));
    end
end
...
%     color = 'k';
    ...
    plott(phi_axis.*180/pi, squeeze(mu_re(1, :, m, n)), xl, yl, txt, 2, dtext, color, marker)
% else
...
if (Nr == 1) && (length(Vmean) == 1)
    ...
    plott(phi_axis.*180/pi, squeeze(sigma_re(1, :, m, n)), xl, yl, txt, 2, dtext, color, marker)
% else
%     txt = ['SNR = ', num2str(db(mean(SNR))/2), ' dB '];
%     xl = 'x [km]';
%     yl = 'y [km]';
...
color = 'k';
yl =  ['\mu retrieved [m.s^{-1}]'];
plott(Vmean, squeeze(mu_re(1, 1, m, :)), xl, yl, txt, 2, dtext, color, marker)
...
color = 'k';
yl =  ['\sigma retrieved [m.s^{-1}]'];
plott(Vmean, squeeze(sigma_re(1, 1, m, :)), xl, yl, txt, 2, dtext, color, marker)
