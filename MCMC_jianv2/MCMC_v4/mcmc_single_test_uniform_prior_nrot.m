...
[accepted, rejected, itern, E_new, sample_MC_seq(l, :), sample_all, Flag, NB(l)] = MHu_uniformprior(E, Num_Simu, Z_avail_vec, t_avail, x0, sigma_n, epsilon);
...
if Flag
    sample_MC(ni, l).seq = sample_MC_seq(l, NB(l)+1:end);
    u_mean_unwraped(ni, l, m) = mean(sample_MC(ni, l).seq);
%     u_mean(ni, l, m) = mod(u_mean_unwraped(ni, l, m), v_amb);
    u_mean(ni, l, m) = u_mean_unwraped(ni, l, m);
...
figure; 
for l = 14
    hold on; plot(Sample_mc_seq_with_burnin(6, 1, l).seq); 
end
