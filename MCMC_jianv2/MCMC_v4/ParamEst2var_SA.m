...
x0 = 2;  % Start position of the target
a = 1;
...
Opt.objective = @(x) Max_LikelihoodGauss(x(1:2), Nscatters_p, Z_avail_vec, t_avail, x0, sigma_n, 1);
...
%      'UseParallel', true, 'MaxFunEval', Inf, 'MaxIter', 1000, 'MeshTolerance', 0, 'StepTolerance', 0);
...
% [Result(k, :), fval(k), exf2(k), out(k)]  = ga(Opt);
% 
% time_consumed(k) = toc;
% % tic;
% % % 
...
tic;
[Result(k, :), fval(k), exf2(k), out(k)] = simulannealbnd(Opt.objective, Opt.x0, Opt.lb, Opt.ub, Opt.options);
time_consumed(k) = toc;
...
%     figure;
    ...
    set(bar_handle, {'DisplayName'}, {'SA algorithm'; 'Ground Truth'});
    legend;
    title(['Histogram of ', var(er).name], 'FontSize', 16);
    xlabel(var(er).name, 'FontSize', 16);
    grid on;
    
    print(['SA_Histogram_var', num2str(er)], '-depsc');
...
print('SA_Doppler_Spectrum', '-depsc');